...
class CMUSkeleton(object):
    ...
    def get_husk_header(self, poses_3d):
        ...
        for joint in self.keypoint2index:
            ...
            nodes[joint] = husk_helper.huskNode(
                name=joint,
        ...
        header = husk_helper.huskHeader(root=nodes[self.root], nodes=nodes)
        return header
    ...
    def poses2husk(self, poses_3d, header=None, output_file=None):
        if not header:
            header = self.get_husk_header(poses_3d)
        ...
        if output_file:
            husk_helper.write_husk(output_file, header, channels)
